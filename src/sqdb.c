...
sqdb_ratesfield_missing(struct SQDB_RateList *lhs, const struct SQDB_RateList *rhs)
{
	...
	for (i=0; i<lhs->count; i++) {
		...
		if (!was_found) {
			result.rates[result.count++] = (unsigned char)rate;
		}
	}
...
static void
event_update_packet(struct SQDB_Event *event, unsigned frame_type, struct StackFrame *frame)
{
    unsigned char *buf;
    unsigned buf_length;
...
sqdb_base_channel(struct SQDB_AccessPoint *entry, unsigned channel)
{
	...
	entry->channels[entry->channel_count++] = (unsigned char)channel;
